...
$statement1 = mysqli_prepare($con1, "SELECT * FROM link_user_group WHERE user_id=? and group_id=?");
mysqli_stmt_bind_param($statement1, "ii", $user_id, $group_id);
mysqli_stmt_execute($statement1);

mysqli_stmt_store_result($statement1);
mysqli_stmt_bind_result($statement1, $user_id, $group_id);
...
if ($response["success"] == true):
    ...
    $statement2 = mysqli_prepare($con2, "DELETE FROM `link_user_group` WHERE user_id=? and group_id=?");
